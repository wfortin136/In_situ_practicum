...
!InSA begin
  ...
  include 'mpif.h'
	interface
     ...
     subroutine cpdata (pid, timestep, fldname, numdims, dimextents, field, com_group) bind(C)
       ...
       integer(C_INT), intent(in) 				:: dimextents(x:y)
     ...
     !  integer(C_INT), intent(in), value :: enddim2
       real(C_DOUBLE), intent(in) 				:: field(startdim1:enddim1,startdim2:enddim2)
  ...
  integer :: stepno														!the time step being analyzed
	integer :: numdims													!number of dimensions of the analysis variable
	integer, dimension (1:4) :: dimextents			!extents of each dimension 
...
!	integer :: stepT
  real(r8) :: startT, endT, diffT, diffT_Global !timekeeping
...
!InSA begin
    if(iam .lt. 6) then
      !write(iulog,*) iam, ' debug idim=', idim, ' ', c, ' ', fname
	  endif
!InSA end
!
...
!InSA begin
  if(fname .eq. 'T') then
     !write(iulog,*) fname, 'Timestep: ', get_nstep(), 'Rank: ', iam, 'Lat I: ', c,'Dim1: ', begdim1, ':', enddim1, 'Dim2: ', begdim2, ':', enddim2, 'Dim3: ', begdim3, ':', enddim3, 'Endi: ', endi, 'dimind', dimind 
    if (iam .le. 1) then
       jeu = enddim2-begdim2+1
       do j=1,jeu
        do i=1,endi
          !write(iulog,*) iam, ' ', fname, '[', i, '][', j, '] = ', field(i,j)
        end do
       end do
    endif
endif
...
!InSA begin
    ...
    !call t_startf ('copyfld')
    startT = MPI_Wtime()
		if(fname .eq. 'PS' .or. fname .eq. 'T' .or. fname .eq. 'U' .or. fname .eq. 'V') then
			!number of dimensions
 			numdims = 2

			!copy dimensions into dim array
			dimextents(1:4) = (/1, endi, begdim2, enddim2/)

			!pointer to field array
    	ieu = endi
    	jeu = enddim2-begdim2+1

    !if (.not. associated(fptr)) allocate (fptr(ieu, jeu))
    !do j=1,jeu
        !do i=1,endi
          !fptr(i,j) = field(i,j) 
        !end do
      !end do
			!fptr => ftarget(1:endi, begdim2:enddim2)
			!cptr = C_LOC (fptr(1,begdim2))

	  	call cpdata (iam, stepno, trim(fname)//NUL, 2, dimextents(1:4), field(1:endi, begdim2:enddim2), mpicom) 
	  	!call copyfld (iam, stepno, trim(fname)//NUL, 1, endi, begdim2, enddim2, field(1:endi, begdim2:enddim2)) 
      !stepT = stepT + 1

			!if (associated(fptr)) deallocate (fptr)
		endif
    !call t_stopf ('copyfld')
    endT = MPI_Wtime()
    diffT = endT - startT

    !call mpi_reduce(diffT, diffT_global, 1, MPI_REAL8, MPI_MAX, 0, mpicom, ierr)
    
		if (stepno .le. 8) then
     write(iulog,*) stepno, ' timeToCopy ', diffT_global
		 if(fname .eq. 'PS' .or. fname .eq. 'T' .or. fname .eq. 'U' .or. fname .eq. 'V') then
    	 ...
    	 jeu = enddim2-begdim2+1
			 ...
			 do j=1,jeu
        do i=1,endi
          !write(iulog,*) iam, ' ', get_nstep(), ' ', fname, '[', i, '][', j, '] = ', field(i,j)
        end do
    	 end do
			endif
