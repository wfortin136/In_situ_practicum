...
*/
  ...
  MPI_Reduce(get_mean_ptr(l_pres), get_mean_ptr(g_pres), 1,MPI_DOUBLE, 
...
/*
  ...
  MPI_Allreduce(get_max_ptr(l_temp), get_max_ptr(g_temp), 1,MPI_DOUBLE, 
      MPI_MAX, MPI_COMM_WORLD);
*/
  MPI_Reduce(get_mean_ptr(l_dens), get_mean_ptr(g_dens), 1,MPI_DOUBLE, 
      MPI_SUM, 0, MPI_COMM_WORLD);
  MPI_Reduce(get_mean_ptr(l_temp), get_mean_ptr(g_temp), 1,MPI_DOUBLE, 
      MPI_SUM, 0, MPI_COMM_WORLD);

  MPI_Reduce(get_min_ptr(l_pres), get_min_ptr(g_pres), 1,MPI_DOUBLE, 
      MPI_MIN, 0, MPI_COMM_WORLD);
  MPI_Reduce(get_min_ptr(l_dens), get_min_ptr(g_dens), 1,MPI_DOUBLE, 
      MPI_MIN, 0, MPI_COMM_WORLD);
  MPI_Reduce(get_min_ptr(l_temp), get_min_ptr(g_temp), 1,MPI_DOUBLE, 
      MPI_MIN, 0, MPI_COMM_WORLD);
  
  MPI_Reduce(get_max_ptr(l_pres), get_max_ptr(g_pres), 1,MPI_DOUBLE, 
      MPI_MAX, 0, MPI_COMM_WORLD);
  MPI_Reduce(get_max_ptr(l_dens), get_max_ptr(g_dens), 1,MPI_DOUBLE, 
      MPI_MAX, 0, MPI_COMM_WORLD);
  MPI_Reduce(get_max_ptr(l_temp), get_max_ptr(g_temp), 1,MPI_DOUBLE, 
      MPI_MAX, 0, MPI_COMM_WORLD);
...
//NEED TO UPDATE HISTO INTERFACE
  ...
  t4 = MPI_Wtime();
  MPI_Reduce(get_histo_array(l_pres), get_histo_array(g_pres), 
      get_histo_numcols(g_pres), MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD);
  
  t5=MPI_Wtime();
  
  MPI_Reduce(get_histo_array(l_dens), get_histo_array(g_dens), 
      get_histo_numcols(g_dens), MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD);
  MPI_Reduce(get_histo_array(l_temp), get_histo_array(g_temp), 
      get_histo_numcols(g_temp), MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD);  
